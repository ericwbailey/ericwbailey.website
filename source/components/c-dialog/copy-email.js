...
function resetCopyButton() {
  ...
  copyButton.innerHTML = '<svg height="14" width="14" aria-hidden="true" viewBox="0 0 100 100" class="icon c-dialog__icon"><use xlink:href="#icon-clipboard"></use></svg>Copy<span class="content-hide-visually"> email address</span>';
}


var clipboard = new Clipboard( '.c-dialog__copy' );

// Adds a success state when the email address is successfully copied
clipboard.on('success', function( e ) {
  copyButton.classList.add( 'is-successful' );
  copyButton.setAttribute( 'aria-live', 'polite' );
  copyButton.innerHTML = '<svg height="14" width="14" aria-hidden="true" viewBox="0 0 100 100" class="icon c-dialog__icon"><use xlink:href="#icon-checkmark"></use></svg><span class="content-hide-visually">Email address </span>Copied';
...
clipboard.on('error', function( e ) {
  var contactField = document.getElementById( 'copy' );
  ...
  contactError.setAttribute( 'class', 'c-dialog__error' );
